  ...
  providers: [
    ...
    // Custom providers for discriminator models
    {
      ...
      useFactory: (userModel) => {
        ...
        if (userModel.discriminators && userModel.discriminators[Patient.name]) {
          return userModel.discriminators[Patient.name];
        }
        return userModel.discriminator(Patient.name, PatientSchema);
      },
    ...
    },
    {
      ...
      useFactory: (userModel) => {
        // Vérifier si le discriminator existe déjà
        if (userModel.discriminators && userModel.discriminators[Medecin.name]) {
          return userModel.discriminators[Medecin.name];
        }
        return userModel.discriminator(Medecin.name, MedecinSchema);
      },
      inject: [getModelToken(User.name)],
    },
    {
      ...
      useFactory: (userModel) => {
        // Vérifier si le discriminator existe déjà
        if (userModel.discriminators && userModel.discriminators[Pharmacien.name]) {
          return userModel.discriminators[Pharmacien.name];
        }
        return userModel.discriminator(Pharmacien.name, PharmacienSchema);
      },
      inject: [getModelToken(User.name)],
    },
  ],
