...
export class AiPredictionService {
  ...
  async predictTrend(
    ...
    // Step 5 — Call Ollama (never throw)
    ...
    try {
      const { data } = await axios.post(
        OLLAMA_URL,
        { model: OLLAMA_MODEL, system: SYSTEM_PROMPT, prompt: userPrompt, stream: false },
      ...
      const p = JSON.parse(match[0]) as Partial<PredictionResult>;
      parsedPrediction = {
      ...
      const reason = axios.isAxiosError(err)
        ...
        : String(err);
      this.logger.warn(`Ollama prediction failed — using fallback. Reason: ${reason}`);
